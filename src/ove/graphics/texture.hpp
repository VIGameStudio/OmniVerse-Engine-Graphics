...
enum wrap_mode { REPEAT, MIRRORED_REPEAT, CLAMP_TO_EDGE, CLAMP_TO_BORDER };
...
enum pixel_channel { RGB, RGBA };

struct texture_t : public resource_t {
  texture_t()
      : m_sWrapMode(wrap_mode::REPEAT), m_tWrapMode(wrap_mode::REPEAT),
        m_minFilter(frag_filter::LINEAR), m_magFilter(frag_filter::LINEAR),
